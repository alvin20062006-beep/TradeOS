...
class TestRiskEngineBasic:
    ...
    def test_exit_bias_triggers_exit(self) -> None:
        ...
        # VolTargeting sizing=750，existing_position=200，clamp 到 200
        ...
        assert plan.final_quantity == 200.0  # clamp 到已有持仓
        assert plan.is_reducing
        assert plan.direction == Direction.SHORT  # 目标剩余暴露 = 空
        assert plan.exec_action == "SELL"       # 执行动作 = 卖出（平多仓）✓

    def test_reduce_risk_maintains_direction(self) -> None:
        """reduce_risk: LONG 持仓 → 减仓，执行动作=卖出（不是买入加多！）"""
        ...
        assert plan.final_quantity == 200.0   # clamp 到已有持仓
        assert plan.is_reducing
        assert plan.direction == Direction.LONG  # 目标剩余暴露 = 多头
        assert plan.exec_action == "SELL"       # 执行动作 = 卖出（减多仓，不是买入！）✓

    def test_reduce_risk_short_exec_action_is_buy(self) -> None:
        """reduce_risk: SHORT 持仓 → 减仓，执行动作=买入（不是卖出开空！）"""
        decision = _decision(bias="reduce_risk", confidence=0.7)
        portfolio = _portfolio(100_000.0)
        ctx = _ctx(price=100.0, adv=1_000_000.0)
        plan = self.engine.calculate(
        ...
        assert plan.final_quantity > 0
        assert plan.is_reducing
        assert plan.direction == Direction.SHORT  # 目标剩余暴露 = 空头
        assert plan.exec_action == "BUY"         # 执行动作 = 买入（减空仓买回，不是开空！）✓
    ...
    def test_exit_bias_short_side(self) -> None:
        ...
        assert plan.is_reducing
        assert plan.direction == Direction.LONG  # 平空仓 = 做多（cover）
        assert plan.exec_action == "BUY"        # 执行动作 = 买入（平空仓）✓

    def test_exit_without_position_returns_zero(self) -> None:
        """exit_bias + 无持仓上下文 → zero_plan"""
        decision = _decision(bias="exit_bias", confidence=0.9)
        portfolio = _portfolio(100_000.0)
        ctx = _ctx(price=100.0, adv=1_000_000.0)
        plan = self.engine.calculate(
            decision, portfolio, market_context=ctx,
            existing_direction=None,  # 无持仓
