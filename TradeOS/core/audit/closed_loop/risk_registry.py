"""Append-only persistence for RiskAudit objects."""
...
class RiskAuditRegistry:
    ...
    def append(self, record: RiskAudit) -> None:
        if not isinstance(record, RiskAudit):
            raise TypeError(f"RiskAuditRegistry.append() requires RiskAudit, got {type(record).__name__}.")
        with open(self._path_for_today(), "a", encoding="utf-8") as f:
            f.write(record.model_dump_json() + "\n")

    def read_all(self, *, symbol: str | None = None, since: datetime | None = None) -> list[RiskAudit]:
        records: list[RiskAudit] = []
            ...
            with open(path, encoding="utf-8") as f:
                for line in f:
                    ...
                    if not line:
                        continue
                    try:
                        record = RiskAudit.model_validate_json(line)
                    except Exception:
                        continue
